...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, avail_action_mask, done):  # 将数据加入buffer
        self.buffer.append((state, action, reward, next_state, avail_action_mask, done))

    def sample(self, batch_size):
        transitions = random.sample(self.buffer, batch_size)
        state, action, reward, next_state, avail_action_mask, done = zip(*transitions)
        return np.array(state), action, reward, np.array(next_state), avail_action_mask, done
...
class DQN:
    ...
    def take_action(self, state, avail_action):
        # epsilon-贪婪策略采取动作
        # up down left right
        avail_action_dim = []
    ...
    def update(self, transition_dict):
        ...
        next_states = torch.tensor(transition_dict['next_states'],
                                   dtype=torch.float).to(self.device)
        avail_action_mask = torch.tensor(transition_dict['avail_action_mask'],
                                    dtype=torch.float).to(self.device)
